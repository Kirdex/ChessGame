...
public class ChessGameRunner 
{
	...
	public void runGame()
	{
		...
		while (true)
		{
			...
			if (turn % 2 == 1) 
				System.out.println("\t(White's Turn) $" + whitePlayer.getMoney());
			else 
				System.out.println("\t(Black's Turn) $" + blackPlayer.getMoney());
			PrintBorder();
			...
			if (turn % 2 == 1)
			{
				...
				moveMade = whitePlayer.run(board, blackPlayer.getChessPieces(), items);
			}
			...
			else
			{
				...
				moveMade = blackPlayer.run(board, whitePlayer.getChessPieces(), items);
			}
	...
	public void createItems()
	{
		...
		items.add(new Scrambler());
		items.add(new Plague());
		items.add(new Reset());
		
		items.add(new QueenGambitOverthrow());
		items.add(new Reinforcement());

		items.add(new InstantPromotion());
		items.add(new UniqueSlayer());

		items.add(new Swapper());
		items.add(new KingTeleport());
