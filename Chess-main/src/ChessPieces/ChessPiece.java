...
{	
	private ArrayList<Movement> movements;			//list of directional movements the piece can make
	private ArrayList<Movement> availableUpgrades;	//list of movements the piece can upgrade to
	private int posRow;								//row position
	private int posCol;								//column position
	private String color;							//piece color
	private boolean isCaptured;						//if the piece is captured
	private boolean hasMoved;						//keeps track of if the piece moved
	private char image;								//piece symbol
	...
	/**
	 * Method to get the piece movements.
	...
	public ArrayList<ArrayList<Character>> calculatePotentialMovements(Board board)
	{
		...
		//Loop through all the movements and add them to the potential movement.
	 ...
	 */
	//Update Chess Pieces' positions depending on where the movement was made to and the type of movement.
			...
			ChessPiece currentChessPiece, ArrayList<ChessPiece> allyChessPieces, ArrayList<ChessPiece> opponentChessPieces)
	{
		...
		if (movementType.equals(Movement.twoUnitsUpSymbol))
		{
			...
			//created as a Pawn object, down-casting it to Pawn only
